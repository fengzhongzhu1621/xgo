...
func NewZkRegDiscv(client *ZkClient) *ZkRegDiscv {
...
func (zkRD *ZkRegDiscv) RegisterAndWatch(path string, data []byte) error {
	log.Infof("register server and watch it. path(%s), data(%s)", path, string(data))
	go func() {
		...
		for {
			...
			if zkRD.registerPath == "" {
				...
				if err != nil {
					log.Errorf("fail to watch register node(%s), err:%s\n", zkRD.registerPath, err.Error())
					if zkRD.zkcli.IsConnectionError(err) {
						log.Error("zk is closed, try to reconnect")
						zkRD.reconnectZk()
						continue
				...
				if !exist {
					...
					log.Errorf("node %s doesn't exist, try to create a new one", zkRD.registerPath)
					zkRD.registerPath = ""
					continue
				}
			}
			...
			select {
			case <-watchCtx.Done():
				log.Infof("watch register node(%s) done, now exist service register.\n", path)
				return
			case e := <-watchEvn:
				log.Infof("watch register node(%s) exist changed, event(%v)\n", path, e)
	...
	log.Infof("finish register server node(%s) and watch it\n", path)
	return nil
}
...
func (zkRD *ZkRegDiscv) reconnectZk() {
	...
	for {
		if zkRD.zkcli.ZkConn != nil && zkRD.zkcli.Ping() == nil {
			log.Info("connection is healthy, there is no need to reconnect, **skip**")
			return
		}
