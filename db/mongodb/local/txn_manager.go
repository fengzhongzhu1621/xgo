...
const (
	transactionNumberRedisKeyNamespace = "transaction:number:"
	transactionErrorRedisKeyNamespace  = "transaction:error:"
)
...
func (t *TxnManager) GenTxnNumber(sessionID string, ttl time.Duration) (int64, error) {
	...
	defer pip.Close()
	if ttl == 0 {
		ttl = db.TransactionDefaultTimeout
	}
...
func (t *TxnManager) PrepareTransaction(cap *db.TxnCapable, cli *mongo.Client) (mongo.Session, error) {
	// create a session client.
	sess, err := cli.StartSession()
	if err != nil {
		return nil, fmt.Errorf("start session failed, err: %v", err)
...
func (t *TxnManager) GetTxnContext(ctx context.Context, cli *mongo.Client) (context.Context, mongo.Session, bool,
	...
	sessCtx := ContextWithSession(ctx, session)
...
func parseTxnInfoFromCtx(txnCtx context.Context) (*db.TxnCapable, bool, error) {
	id := txnCtx.Value(db.TransactionIdHeader)
	if id == nil {
	...
	// parse timeout
	ttl := txnCtx.Value(db.TransactionTimeoutHeader)
	if ttl == nil {
...
func (t *TxnManager) setTxnError(sessionID sessionKey, txnErr error) {
	...
	case strings.Contains(txnErr.Error(), "WriteConflict"):
		...
		if err != nil {
			log.Errorf("set txn error(%v) failed, err: %v, session id: %s", txnErr, err, sessionID)
		}
	default:
	}
}
...
func (t *TxnManager) GetTxnError(sessionID sessionKey) TxnErrorType {
	...
	if err != nil && redis.Nil == err {
		log.Errorf("get txn error failed, err: %v, session id: %s", err, sessionID)
		return UnknownType
	}
...
func GenTxnCableAndSetHeader(header http.Header, opts ...db.TxnOption) (*db.TxnCapable, error) {
	...
	if len(opts) != 0 {
		if opts[0].Timeout < 30*time.Second {
			timeout = db.TransactionDefaultTimeout
		} else {
	...
	header.Set(db.TransactionIdHeader, sessionID)
	header.Set(db.TransactionTimeoutHeader, strconv.FormatInt(int64(timeout), 10))
