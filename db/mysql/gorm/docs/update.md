# Save
会保存所有的字段，即使字段是零值
NOTE: 不要将 Save 和 Model一同使用, 这是 未定义的行为。
...
# 阻止全局更新
执行一个没有任何条件的批量更新，GORM 默认不会运行，并且会返回 ErrMissingWhereClause 错误
```go
...
# 不使用 Hook 和时间追踪
更新时跳过 Hook 方法，并且不追踪更新的时间，你可以使用 UpdateColumn, UpdateColumns, 它们的用法类似于 Update, Updates
```go
