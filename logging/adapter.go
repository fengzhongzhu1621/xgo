...
func NewStdLogger(debug, trace bool, prefix string) LoggerAdapter {
	return NewStdLoggerWithOut(os.Stderr, debug, trace, prefix)
}

// NewStdLoggerWithOut creates StdLoggerAdapter which sends all logs to provided io.Writer.
func NewStdLoggerWithOut(out io.Writer, debug bool, trace bool, prefix string) LoggerAdapter {
	l := log.New(out, prefix, log.LstdFlags|log.Lmicroseconds|log.Lshortfile)
...
// 捕获所有的日志，将消息放到字典中，仅用于测试
type CaptureLoggerAdapter struct {
	captured map[LogLevel][]CapturedMessage // 存放捕获的消息
	fields   LogFields                      // 最近一次的日志字段
	lock     sync.Mutex
}
