...
// LogFunc 定义日志输出接口，需要使用方自定义输出函数的实现.
type LogFunc func(logScope *LogScope, level level.LogLevel, levelName string, category string, msg string, args ...interface{})

type LogScope struct {
	Parent     *LogScope      // Parent scope
	MinLevel   level.LogLevel // Minimum log level to write out.
...
func RootLogScope(minLevel level.LogLevel, logFunc LogFunc) *LogScope {
	return &LogScope{
		Parent:     nil,
		MinLevel:   minLevel,
		Mutex:      &sync.Mutex{},
...
func LevelFromString(s string) level.LogLevel {
	switch s {
	case "debug":
		return LogDebug
	case "trace":
