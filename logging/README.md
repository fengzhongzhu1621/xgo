...
func main() {
    ...
    logger := With(Field{Key:"user", Value: os.Getenv("USER")})
...
```go
dl := GetDefaultLogger()
l := dl.With(Field{Key: "user", Value: os.Getenv("USER")})
l.Info("hello, world")
```
...
2. 底层严重问题才会打印 trace 日志，需要设置环境变量才会开启：export XGO_LOG_TRACE=1
...
```shell
export XGO_LOG_TRACE=1
./server -conf path/to/xgo.yaml
```
