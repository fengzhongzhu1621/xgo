...
type Carbon struct {
	// 完全兼容标准库的time.Time类型，实际上该库的日期时间类型Carbon直接将time.Time内嵌到结构中，
	// 所以time.Time的方法可直接调用
	time.Time
	weekStartsAt time.Weekday   // 星期开始
	weekEndsAt   time.Weekday   // 星期结束
	weekendDays  []time.Weekday // 周末
	stringFormat string         // 日期默认格式
...
func NewCarbon(t time.Time) *Carbon {
	...
	return &Carbon{
		Time:         t,
		weekStartsAt: time.Monday,   // 一个星期从星期一开始
		weekEndsAt:   time.Sunday,   // 星期日结束
		weekendDays:  wds,           // 周末默认是星期六和星期日
		stringFormat: DefaultFormat, // 日期的默认格式
		Translator:   translator(),  // 默认翻译器是英文
	}
}
...
// After will be behaved like time.After unless time has been frozen
...
// Tick will be behaved like time.Tick unless time has been frozen
...
// time.Unix: 返回本地时间，该时间等于1970年1月1日在UTC开始的Unix时间。
