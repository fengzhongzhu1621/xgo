...
func NewStickyConnPool(pool Pooler) *StickyConnPool {
	// 判断pool是否是共享连接池，只创建一次
	p, ok := pool.(*StickyConnPool)
	if !ok {
		p = &StickyConnPool{
			pool: pool,
			ch:   make(chan *Conn, 1), // 仅包含一个共享连接
		}
	}
	// 设置连接的共享次数
...
func (p *StickyConnPool) Get(ctx context.Context) (*Conn, error) {
	...
	for i := 0; i < 1000; i++ {
		...
		case stateDefault:
			...
			// 状态变更失败， 从连接池中删除获取的唯一连接，并关闭新建的连接
