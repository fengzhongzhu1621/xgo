...
func TestUnitServerFilter(t *testing.T) {
	Convey("TestUnitServerFilter with default config", t, func() {
		serverFilter := ServerFilterWithOptions(defaultOptions)

		Convey("test invalid", func() {
			req := new(struct{})
			rsp, err := serverFilter(context.Background(), req, serverHandler(&struct{}{}, nil))
			So(err, ShouldBeNil)
		...
		Convey("test validate failed with ctx", func() {
			...
			rsp, err := serverFilter(ctx, req, serverHandler(nil, errors.New("err fake")))
			So(err, ShouldNotBeNil)
			So(rsp, ShouldBeNil)
			So(errs.Code(err), ShouldEqual, errs.RetServerValidateFail)
		})
