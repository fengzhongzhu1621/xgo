...
func startPutMsgs(r *Ring[[]byte], wg *sync.WaitGroup, num int) {
	for {
		if num <= 0 {
			break
		}
...
func startGetMsgs(r *Ring[[]byte], wg *sync.WaitGroup, num int) {
	for {
		if num <= 0 {
			break
		}
